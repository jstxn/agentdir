...
then re-runs adoption for the current git repository. It refreshes hooks and the
Codex skill by default, then runs `doctor`.
...
agentdir --upgrade --upgrade-version v0.5.3
...
```bash
AGENTDIR_WHEEL=/path/to/agentdir-0.5.3-py3-none-any.whl bash scripts/install.sh
```

To force the virtual environment installer even when `pipx` is present:

```bash
AGENTDIR_FORCE_VENV=1 AGENTDIR_WHEEL=/path/to/agentdir-0.5.3-py3-none-any.whl bash scripts/install.sh
```
...
```bash
gh api -H "Accept: application/vnd.github.raw" \
  'repos/jstxn/agentdir/contents/scripts/rollback.sh?ref=v0.5.3' | bash -s -- v0.5.2
```
...
```bash
gh api -H "Accept: application/vnd.github.raw" \
  'repos/jstxn/agentdir/contents/scripts/install.sh?ref=v0.5.2' | AGENTDIR_VERSION=v0.5.2 bash
```
...
- `agentdir-0.5.3-py3-none-any.whl`
- `agentdir-0.5.3.tar.gz`
- `install-agentdir.sh`
