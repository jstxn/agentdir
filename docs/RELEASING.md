...
## Publish The Pi Package

PyPI is automated; npm is not. The Pi package `@jstxn/agentdir-pi` has to be
published by hand or Pi users stay on the previous version:
...
```bash
npm view @jstxn/agentdir-pi version
```
