...
```text
dist/agentdir-0.3.2-py3-none-any.whl
dist/agentdir-0.3.2.tar.gz
...
```bash
git tag -a v0.3.2 -m "Release AgentDir v0.3.2"
...
```bash
tmp="$(mktemp -d)"
gh release download v0.3.2 --repo jstxn/agentdir --pattern install-agentdir.sh --dir "$tmp"
